...
| IMAP bridge (IDLE)               | ``wait_for_greenmail_inbox_message_seen_host`` — якорное письмо в INBOX с ``\\Seen`` (на сервере, после FETCH бриджа); альтернатива ``*_gone(_host)`` по UNSEEN    |
