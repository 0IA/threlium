...
> **Двойное устаревание исторических формулировок (старая схема с archive → нынешняя; GraphRAG → LightRAG).** Матрицы переходов ниже — единственный нормативный артефакт этого документа и сохраняются 1:1; supporting prose адресует одновременно два слоя legacy: (1) «`cc "$ARCHIVE"` + выделенный `archive/Maildir`» → **fdm** `notmuch insert …` + durable `stages/<stage>/Maildir/cur/<id>:2,S` под единым union notmuch root; (2) «GraphRAG `input/`-каталоги + симлинк `input/global/` → `graphrag/global/` + мутабельный тег `graphrag_exported`» → один общий `LightRAG(working_dir=…)` + ingest: оболочка `EmailMessage`/`policy.default`, тело `lightrag/ingest_body.j2`, `X-Threlium-Thread-Id` только в синтетической строке `ainsert` + тег `lightrag_indexed` ([`INDEX.md` §5b](INDEX.md#5b-lightrag-worker), [§7.6](INDEX.md#76-per-thread-scoping-soft-через-маркеры), [ADR 0001](adr/0001-lightrag-ingest-chunking-enrich.md)).
...
Так как мы не покидаем текущего RFC-треда агента, кросс-тредовая маршрутизация не нужна. Это простая линейная петля с возвратом в `ingress`.
...
| **2** | `thread_memory` | **Получает:** `L_M1`<br>**Отправляет:** `L_M2`<br>• `To: ingress@`<br>• `In-Reply-To: <L_M1>` | Скрипт получает письмо, при необходимости форматирует тело (Markdown/JSON) и собирает RFC через `render_prompt("thread_memory/base.j2", …)` — без `[Scope:]` в теле и без `X-Threlium-Thread-Id` на диске ([`INDEX.md` §7.6](INDEX.md#76-per-thread-scoping-soft-через-маркеры)). Само попадание `L_M1` в этот узел уже атомарно записано **fdm**/**notmuch insert** в `stages/thread_memory/Maildir/new/` и проиндексировано в union notmuch (без отдельного `cc archive`). Воркер после успешного emit `L_M2` делает `nm_settle(L_M1)` ([`INDEX.md` §5.5.3](INDEX.md#553-notmuch-consistency-через-notmuch2mutabletagset)) — оригинал переезжает `new/<id>` → `cur/<id>:2,S`; LightRAG-воркер по cur/-PathChanged-триггеру индексирует его в общий граф (синтетическая ingest-строка с thread id). Возврат потока в единую точку входа. |
| **3** | `ingress` | **Получает:** `L_M2`<br>**Отправляет:** `L_M3`<br>• `To: enrich@`<br>• `In-Reply-To: <L_M2>` | `ingress` видит обычное продолжение треда (Правило 4: есть In-Reply-To, нет HITL-возврата). Передает поток в `enrich`. |
| **4** | `enrich` | **Получает:** `L_M3` | `enrich` собирает `unified_messages` и цепочку Jinja+LLM+`rag.aquery` ([`INDEX.md` §7](INDEX.md#7-enrich-notmuch-context--query--lightrag), [§7.5](INDEX.md#75-query-call-always-on)). Поскольку `L_M1` и `L_M2` уже settled и проиндексированы в общий граф (синтетический ingest с thread id), локальная заметка доступна через retrieval; приоритет текущего треда — в промптах (`lightrag/rag_response.j2` и сформулированный запрос). |
...
| **2** | `global_memory` | **Получает:** `L_G1`<br>**Отправляет:** `L_G2`<br>• `To: ingress@`<br>• `In-Reply-To: <L_G1>` | Скрипт нормализует тело (Markdown/JSON) и собирает письмо через `render_prompt("global_memory/base.j2", …)` ([`INDEX.md` §7.6](INDEX.md#76-per-thread-scoping-soft-через-маркеры)). `L_G1` уже атомарно записан **fdm**/**notmuch insert** в `stages/global_memory/Maildir/new/` (штатное имя файла Maildir) и проиндексирован в union notmuch. Воркер после успешного emit `L_G2` делает `nm_settle(L_G1)` — оригинал переезжает в `cur/<id>:2,S`; LightRAG-воркер по cur/-PathChanged-триггеру индексирует его в общий `lightrag/working_dir/` (тот же граф, что и thread_memory). Возврат потока в единую точку входа. |
| **3** | `ingress` | **Получает:** `L_G2`<br>**Отправляет:** `L_G3`<br>• `To: enrich@`<br>• `In-Reply-To: <L_G2>` | Обычное продолжение треда (Правило 4 `ingress_router`: есть `In-Reply-To`, RA tail → From не `cli_hitl_out` → не-HITL). |
| **4** | `enrich` | **Получает:** `L_G3` | Цепочка `unified_messages` и `aquery` как в [`INDEX.md` §7.5](INDEX.md#75-query-call-always-on). Глобальные факты и тредовый контекст живут в одном графе; отбор релевантности — семантика retrieval + инструкции в Jinja, без отдельного notmuch-мульти-запроса по `[Scope:]`. |
...
`reflect` — стадия-tool, доступная `reasoning` так же, как `thread_memory` и `global_memory`, но семантика принципиально иная: **это не запись факта в память, а явное намерение «думать ещё один цикл»** через свежий проход `ingress → enrich → reasoning` с обновлённым LightRAG-контекстом. С точки зрения графа FSM это такая же одностадийная петля «`X → ingress`», как у memory-стадий, но без какого-либо архивного смысла записи: попадание в durable `stages/reflect/Maildir/` через **fdm** `pipe` (`notmuch insert …`) — общесистемное свойство ([`INDEX.md §4`](INDEX.md#4-mailfilter-terminating-insert)), у `reflect` нет специальной интерпретации со стороны индексации (тот же общий граф и тот же ingest-пайплайн, что и для остальных settled писем треда).

Главное отличие от `thread_memory` — **budget-aware промпт**: `reflect` читает остаток `X-Threlium-Hop-Budget`, выбирает Jinja2-шаблон из подкаталога `$THRELIUM_HOME/prompts/reflect/` (`continue.j2`, пока бюджета хватает на ещё один цикл `reflect → ingress → enrich → reasoning`; `final.j2`, когда бюджет почти исчерпан) и подставляет конкретное число оставшихся hop'ов в результирующий текст. На следующем витке `reasoning` модель *видит* свой ресурс и может скорректировать стратегию (продолжать копать или закругляться). У `thread_memory` тело статичное — чувствительности к бюджету там нет и быть не должно.
...
| **2** | `reflect` | **Получает:** `R_R1`<br>**Отправляет:** `R_R2`<br>• `To: ingress@`<br>• `In-Reply-To: <R_R1>`<br>• `Body:` рендер `continue.j2` или `final.j2` с подстановкой `remaining_hops`, `cycle_cost`, `previous_reasoning`, `subject` | Стадия читает хвост `X-Threlium-Hop-Budget` (`N` → остаток `N`; `N-k` → остаток `max(0, N-k)`), выбирает шаблон по правилу `remaining ≥ cycle_cost (3) + safety_margin (1)` и кладёт результат в text/plain. После успешного emit `R_R2` воркер делает `nm_settle(R_R1)` — оригинал переезжает `new/<id>` → `cur/<id>:2,S`, ровно как у любой стадии ([`INDEX.md §5.5.3`](INDEX.md#553-notmuch-consistency-через-notmuch2mutabletagset)). |
| **3** | `ingress` | **Получает:** `R_R2`<br>**Отправляет:** `R_R3`<br>• `To: enrich@`<br>• `In-Reply-To: <R_R2>` | Обычное продолжение треда (Правило 4 `ingress_router`: есть `In-Reply-To`, не-HITL возврат). |
| **4** | `enrich` | **Получает:** `R_R3`<br>**Отправляет:** `R_R4`<br>• `To: reasoning@`<br>• `In-Reply-To: <R_R3>` | Сбор контекста и цепочка `rag.aquery` как в [`INDEX.md` §7](INDEX.md#7-enrich-notmuch-context--query--lightrag) / [§7.5](INDEX.md#75-query-call-always-on). Контекст инжектируется поверх рендеренного reflect-промпта. На следующем reasoning-шаге модель видит и budget-инструкцию, и обновлённый блок обогащения из `enrich`. |
...
3. **Фреймы субагентов не трогаются:** `X-Threlium-Capabilities` пробрасывается билдером `build_fsm_plain_to_stage` — `reflect` не открывает и не закрывает фреймов делегирования. Делегирование остаётся прерогативой `subagent_intent` / `subagent_end` ([`SUBAGENT_TABLE.md`](SUBAGENT_TABLE.md)).
