...
> **⚠ Это легаси-паттерн (`hasNotProperty`). Целевой (детег) — ПОЗИТИВНЫЙ счётчик фазы** `property: {
> phase: { equalTo: "N" } }` + `recordState phase:=N+1`, БЕЗ негативных предикатов: `hasNotProperty`/
> `doesNotContain` порождают комбинаторный взрыв (надо перечислять все исключаемые случаи). AND нескольких
> позитивных предикатов подтверждён в исходниках. См. **§3.6.8** (механика, container-proof, источники).
...
### 3.6.8. Многошаговые/сложные тесты на ОДНОМ generic-стабе — фазовые флаги + seeded-ответы ⭐⭐

**Многошаговый тест не требует per-test стабов.** Состояние шагов («фазы») живёт в **per-test
thread-root контексте**, поэтому ОДИН generic-стаб-набор обслуживает ВСЕ тесты: каждый шаг гейтится по
**ПОЗИТИВНОМУ значению счётчика фазы**, ключ — ЧИСТЫЙ thread-root (tag-free, без `stub_tag`).

**⛔ Только ПОЗИТИВНЫЕ условия — `hasNotProperty` НЕ применяется (нормативно).** Негативные предикаты
(`hasNotProperty`, `doesNotContain`) порождают **комбинаторный взрыв**: чтобы стаб НЕ сработал в чужих
случаях, пришлось бы перечислять КАЖДЫЙ исключаемый случай (и любой новый тест/фаза ломает развязку).
Поэтому фазы развязываем **позитивным счётчиком**: одно свойство `phase` со ЗНАЧЕНИЕМ, и шаг гейтится
`property: { phase: { equalTo: "N" } }` (ровно текущая фаза), а `recordState` продвигает `phase := N+1`.
Шаги становятся **disjoint** на ОДНОМ позитивном равенстве — без `hasNotProperty`, без `priority` (§3.3).

**AND подтверждён в исходниках (vendored).** Несколько предикатов в одном flat-объекте `parameters`
агрегируются через AND: `StateRequestMatcher.getMatchers` собирает ВСЕ предикаты →
`calculateMatch` → `MatchResult.aggregate` → `WeightedAggregateMatchResult.isExactMatch()` =
`matchResults.stream().allMatch(::isExactMatch)` (точный матч ТОЛЬКО если ВСЕ под-предикаты точны).
`property` матчится через `StringValuePattern.match` (позитивное сравнение значения). Явные `and`/`or`/`not`
— массивами (`StateRequestMatcher` ветки `And`/`Or`/`Not`). Источники:
`vendor/wiremock/wiremock-state-extension/.../requestmatcher/StateRequestMatcher.java` (:66, :157-177, :214-227),
`vendor/wiremock/.../matching/WeightedAggregateMatchResult.java` (:41-46).

**Container-proven (2026-06-14, живой WireMock, ПОЗИТИВНЫЙ счётчик).** seed кладёт `phase="0"` + per-phase
ответы; шаг гейтится `property phase==N`, echo'ит `rN`, продвигает `phase:=N+1`. Две независимые сессии
(`X-TR=AAA`/`BBB`):

| вызов | ответ | смысл |
| --- | --- | --- |
| AAA #1 | `STEP0:A0` | `phase==0` → шаг 0, **seeded** ответ AAA, `phase→1` |
| BBB #1 | `STEP0:B0` | независимая сессия, СВОЙ seeded ответ |
| AAA #2 | `STEP1:A1` | `phase==1` → шаг 1; BBB не тронута |
| AAA #3 | `not matched` | `phase==2`, шага нет (чистый конец) |
| BBB #2 | `STEP1:B1` | BBB всё ещё `phase==1`, независимо |

Разные тесты = разные thread-root = разные контексты (§2) → нет cross-talk; счётчик `phase` у каждого свой.

**Механика шага N (нормативная):**
- **Гейт:** `hasContext({{request.headers.[X-Threlium-Thread-Root]}})` + `property: { phase: { equalTo: "N" } }`
  (ровно текущая фаза — позитивно). Контекст с `phase="0"` (+ `active`) сидит `prepare`/setup-стаб ДО контура.
- **Запись (advance):** `recordState → state { phase: "N+1" }` в тот же thread-root контекст + (отдельным
  листенером) `list.addLast { cs: … }` для call-site-учёта (§3.6.1).
- **Ответ шага:** `response-template` echo из per-test seeded-state:
  `"…{{state context=request.headers.[X-Threlium-Thread-Root] property='phase_N_resp' default='…'}}…"`.
  Тест в setup сидит свои per-phase ответы в СВОЙ контекст. Так ОДИН generic-reasoning-набор проигрывает
  СЦЕНАРНЫЙ поток ЛЮБОГО теста — скрипт и счётчик теста лежат в его контексте, у соседей свои. Это
  коллапсирует per-test reasoning-фазовые JSON (319 стабов) в generic-набор.

**Сложные/многофакторные условия — комбинируй ПОЗИТИВНЫЕ предикаты (AND, подтверждён выше):**
- **AND:** несколько позитивных предикатов в одном flat-объекте — напр. `property phase==2` И
  `property gate=="passed"` (оба — позитивные значения, выставленные прошлыми шагами/content-flags).
- **OR:** массивом `"or": [ {…}, {…} ]` (§3.2) — если шаг допускает несколько позитивных предусловий.
- **Контентный фактор (позитивный):** прошлый шаг content-flag'ом (§3.6.2) выставляет ПОЗИТИВНОЕ свойство,
  если запрос НЁС маркер (`{{#if (contains request.body 'MARKER')}}…ставит gate=passed…{{/if}}`); следующий
  шаг гейтится `property gate=="passed"`. Условие = порядок (`phase`) + содержимое (позитивный флаг) +
  структура (`call-site`, §2.5) — всё ПОЗИТИВНО.

**Конкурентность.** Reasoning-фазы ПОСЛЕДОВАТЕЛЬНЫ на handler-треде (один воркер на тред, §1) → запись
`phase` не гонится, `state`-set безопасен. На КОНКУРЕНТНЫХ стадиях (lightrag-drain, §3.6.3) обязателен
append-only single-writer. Выбор идиомы — по конкурентности стадии.

**ПЕРЕМЕННОЕ число reasoning-хопов — ABSORBING терминальная фаза (`207`).** ⭐ Фаза-счётчик хорош, когда
число reasoning-вызовов ИЗВЕСТНО (одиночный mailflow). Но РЕАЛЬНЫЙ агентный клиент (Cline) или multi-turn
тред дают ПЕРЕМЕННОЕ число хопов → счётчик «упирается» в незасиженную фазу → стаб эхает пустой
`function.name` → `RuntimeError: reasoning: tool_call without function.name` → краш воркера → застрявший
`unread` → ТАЙМ-АУТ teardown-drain-барьера (тело теста проходит, teardown ERROR). РЕШЕНИЕ: `207_..phase_7`
сделан ABSORBING — `recordState` ставит `phase:=7` (а не 8) → фаза 7 неподвижная точка, стаб возвращает
`p7` БЕСКОНЕЧНО. Сид с запасом: `[<первый tool>] + [finalize]*7` → после скриптовых фаз каждый лишний хоп =
`finalize` (как старая защёлка «после tasks_upsert — finalize навсегда»). Тесты с фиксированным числом хопов
фазы 7 не достигают → не затронуты. Правило: тест с НЕдетерминированным числом reasoning-вызовов
(реальный клиент / открытый multi-turn) сидит padded-фазы до запаса; иначе достаточно точного перечисления.

**АВТО-ПАДДИНГ до 8 фаз — ОБЯЗАТЕЛЕН под -n12 (2026-06-15).** ⚠ absorbing-``207`` спасает ТОЛЬКО на фазе 7;
если тест засеял K<8 фаз, фазы K..6 ПУСТЫ → лишний reasoning-хоп туда → пустой ``function.name`` → краш
воркера → reply-timeout. Под -n12 даже «детерминированные» тесты ловят ЛИШНИЕ хопы (gate-ретрай / FSM
re-complete / транзиент под нагрузкой — это нормальное продуктовое поведение, как старая защёлка его
поглощала). Поэтому ``wiremock_seed_reasoning_phases`` ТЕПЕРЬ авто-дублирует ПОСЛЕДНЮЮ фазу до p7 (все
конвертированные оканчиваются на ``response_finalize`` — терминальный) → любой overflow-хоп = finalize,
absorbing-207 держит хвост. Предусловие: последняя засеянная фаза терминальна. Это устранило -n12-краши
formal_reason×4 (наблюдались как «деградация»: прогон 5:33 vs 2:00 + greenmail-timeout). Точное перечисление
фаз всё равно нужно для ПЕРВЫХ K (скриптовая логика); паддинг — только страховка хвоста.

**ВЕТВЯЩИЙСЯ reasoning (ретрай: сначала неверный тул / усечение, потом корректно) — STATE-условный ОТВЕТ.** ⭐
(ЗАМЕРЕНО 2026-06-15). Тело ответа стаба condition на липком счётчике: `recordState` инкрементит `retry_n`
(`{{math (state … property='retry_n' default='0') '+' 1}}`), а ответ — `{{#if (eq (state … property='retry_n'
default='0') '0')}}<неверный_тул_или_усечение>{{else}}<корректный_ответ>{{/if}}`. Проверено: call1→WRONG,
call2/3→CORRECT. Так reasoning_mock 098 (wrong-tool-retry) / 099 (length-truncated) делаются детерминированно
БЕЗ журнала: продукт ретраит на неверный/усечённый ответ, на 2-м вызове получает корректный. Счёт ретраев —
по `retry_n` (state), не по журналу.

**Стоимость записи в state под -n12 — ЗАМЕРЕНО (2026-06-15), глобальный замок НЕ узкое место.** ⭐
Vendored state-extension сериализует ВСЕ `recordState` одним глобальным замком (`TransactionManager.
withTransaction` → `synchronized (store)`, TransactionManager.java:39) — на весь Store, не per-context.
Замер на живом контейнере (реалистичный стаб: cs-list + phase + sticky content-flag, нарастающая
конкурентность, 24 thread-root контекста ≈ разброс -n12):

| conc | mean ms | p95 ms | **req/s** |
|---|---|---|---|
| 1  | 4.7  | 11.9 | 213 |
| 12 | 7.6  | 11.2 | **1551** |
| 24 | 14.1 | 21.3 | 1640 |
...
Замок сериализует (плато ~**1600 req/s**, латентность растёт с конкурентностью), НО 1600/с ≫ реального
темпа сьюта (≈12 тестов × ~15 LLM-вызовов / ~40 с ≈ **~5 recordState/с** под -n12, ~300× запас). Замок
свободен ~99.7% времени → **НЕ bottleneck**. ВЫВОД: класть content-flags на ОБЩИЕ generic-стабы (§3.6.3,
«один стаб — флаги у тестов свои») БЕЗОПАСНО, локализовать флаги per-test ради замка НЕ нужно. Правило:
потолки производительности — ЗАМЕРЯТЬ на живом контейнере, не теоретизировать (ср. §3.6.4).

**Пример (memory_query, 3 фазы).** В generic-виде: seed `phase="0"`; шаг `phase==0` → `memory_query`
(`phase→1`); шаг `phase==1` → `tasks_upsert` (`phase→2`); шаг `phase==2` → `response_finalize` (`phase→3`).
Гейт по ЧИСТОМУ thread-root, ответы — seeded-echo, счётчик — в thread-root контексте; per-test JSON
исчезают, `stub_tag` уходит. (Текущие per-test reasoning-стабы ещё используют `hasProperty/hasNotProperty`
— это легаси-паттерн §3.3, заменяется позитивным счётчиком при детеге.)
