...
**Прод vs e2e — call-site всегда штампится.** Базовый маркер `lightrag_query` для query-вызова
ставится `build_query_correlation` (`[runners/lightrag/aquery.py](../ansible/roles/threlium/files/scripts/threlium/runners/lightrag/aquery.py)`)
независимо от `e2e.litellm_route_correlation` — иначе `detect_lightrag_call_site_wire` дефолтнул бы
к базе `lightrag_index` и вернул `extract_knowledge_graph` вместо `generate_rag_answer` (wire-мусор в
`## Answer`). Различие e2e: при флаге к correlation **дополнительно** подмешиваются route-заголовки
(`From/To/Message-ID/X-Threlium-Route`) для WireMock `hasContext` — на проде этого merge нет
(`merge_litellm_call_kwargs_and_log` возвращает kwargs без HTTP-заголовков), call-site используется
лишь для выбора фазы внутри `llm_func`.
...
(`THRELIUM_E2E_SESSIONFINISH_DRAIN_SEC`) — в логе видны `poll(backoff) progress: sut: threlium-work@ / threlium-sweep@`. После FAIL используется
