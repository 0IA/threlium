...
**Ответы LiteLLM (не msgspec):** схема chat/embedding/rerank на стороне провайдера — готовые модели `litellm.types.utils` (`ModelResponse`, `Message`, `EmbeddingResponse`, …) и `litellm.types.rerank` (`RerankResponse`); узкое приведение в [`threlium/litellm_wire.py`](ansible/roles/threlium/files/scripts/threlium/litellm_wire.py). **Контракт embedding-батча:** запрос несёт `input: list[str]`, ответ — `data[i].embedding` строго **по одному вектору на вход в исходном порядке** (`index == i`). Порядок load-bearing: на retrieval LightRAG шлёт батч `[query, ll_keywords, hl_keywords]` и берёт `ll_embedding = all_embeddings[1]` (2-й вход) для entity-поиска — любой продьюсер/стаб, отдающий вектор не на каждый вход (напр. ноль для входов 2..N), вырождает query-вектор и обнуляет retrieval (e2e-проявление — `docs/E2E.md` §3.6.5). Эталон L0-поведения для паритета e2e-стабов — [`tests/e2e/reference_l0/threlium_e2e_l0.py`](../tests/e2e/reference_l0/threlium_e2e_l0.py) (не compose-сервис): декодирует каноничные `@localhost` Message-ID локально (без импорта `threlium.types`), в согласовании с `RfcMessageIdWire.native_from_canonical_str` **только для `EmailNativeId`**. `TelegramNativeId` и `MatrixNativeId` в sidecar не декодируются (разная JSON-схема payload). Это единственное легитимное место с прямым `base62.decodebytes` вне `threlium.types`.
...
**Канал `isomorph` (HTTP-мост):** `IngressRoute` и `NativeId` расширены, `TNative` включает новый native. :class:`~threlium.types.identity.IsomorphIngressRoute` (`channel`/`request_id`/`api_surface`/`model`/`stream`) — `api_surface` (:class:`~threlium.types.identity.IsomorphApiSurface`) выбирает вендорный wire-формат, не slug канала; `model` эхо-возвращается клиенту (Cline берёт лимит контекстного окна), не выбор LLM. :class:`~threlium.types.identity.IsomorphContentId` (`v`, `content_hash`) — **контент-адресуемая** identity: `Message-ID = canon(IsomorphContentId(hash(контент)))`, поэтому мост восстанавливает `In-Reply-To` следующего хода пересчётом хеша last-assistant **без чтения notmuch** (см. [THREAD_MODEL §6](THREAD_MODEL.md#6-канал-isomorph-контент-адресуемые-message-id-glue-без-внешнего-mid)). Хеш — :class:`~threlium.types.isomorph_content.IsomorphContentHashWire` (sha256-hex; фабрики `from_content` / `from_ingress_tail`; публичной `str → hash` нет, прецедент `IrtHashWire`); нормализация ассистент-контента — `IsomorphAssistantContent` / `IsomorphToolCallSig` + `canonical_json` (общий модуль для egress и моста, байтовый паритет MID). Bridge→ingress payload — **только `<system>`** (как у прочих мостов): хвост истории кладётся в `<system>`, `<user-query>` на bridge-input запрещён.
