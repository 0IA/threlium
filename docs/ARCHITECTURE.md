...
- **Почта (`threlium.bridges.email`):** long-running IMAP IDLE bridge (`threlium-bridge@email.service` → `python -m threlium.runners.bridge email`); UNSEEN → notmuch-дедуп по каноническому `Message-ID` → полный fetch при новизне → канонизация MIME (stdlib `email`, `canonicalize_mime`) → **`run_fdm`**. Без fetchmail и stdio-конвейера.
...
Всё формирование пользовательского/LLM-видимого контента писем (тела, динамические Subject, FSM/wire-format маркеры внутри тел) вынесено в Jinja2-шаблоны под раскладкой по стадиям FSM `$THRELIUM_HOME/prompts/<stage>/<purpose>.j2` (например, `reflect/continue.j2`, `reflect/final.j2`, `ingress/orphan_notice.j2`, `cli_intent/*.j2`, `cli_resume/*.j2`, `cli_hitl_out/*.j2`, `cli_exec/observation.j2`, `subagent_intent/budget_exhausted.j2`, `reasoning/system.j2`, `reasoning/user.j2`, `reasoning/<route>/{tool_spec,email_body,email_subject}.j2`, `lightrag/ingest_body.j2`, `lightrag/enrich_*.j2`, `lightrag/mail_context.j2`). Конверт bridge→`ingress@` собирается в Python (`threlium.bridges.build_bridge_ingress_email`), не через Jinja.
...
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| `threlium.bridges.email` | Long-running IMAP IDLE bridge (`imap-tools`): UNSEEN → notmuch-дедуп → полный fetch → канонизация → **`run_fdm`** (через раннер). | `python -m threlium.runners.bridge email` (systemd `threlium-bridge@email.service`) |
