...
| `cli_exec@localhost`   | **Только исполнение**; sandbox (`systemd-run --user` + `ProtectSystem=strict`) или privileged (`--uid=0`); observation → `enrich_fast@localhost`.                                                            |
