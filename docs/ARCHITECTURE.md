...
- **Почта (`threlium.bridges.email`):** long-running IMAP IDLE bridge (`threlium-bridge@email.service` → `python -m threlium.runners.bridge email`); IMAP UID-watermark (`imap_uid` в `X-Threlium-Route`, читается из notmuch) → `UID SEARCH UID <wm+1>:*` → notmuch-дедуп по каноническому `Message-ID` → полный fetch при новизне → канонизация MIME (stdlib `email`, `canonicalize_mime`) → **`run_fdm`**. Без fetchmail и stdio-конвейера.
...
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| `threlium.bridges.email` | Long-running IMAP IDLE bridge (`imap-tools`): UID-watermark (`imap_uid` из notmuch) → `UID SEARCH UID <wm+1>:*` → notmuch-дедуп → полный fetch → канонизация → **`run_fdm`** (через раннер). | `python -m threlium.runners.bridge email` (systemd `threlium-bridge@email.service`) |
