...
| [E2E.md](E2E.md)                   | e2e-контракт плейбука: Testcontainers + Compose + GreenMail + WireMock (OpenAI HTTP mock).                                                |
...
**Единственный quality gate плейбука — e2e-тесты** ([E2E.md](E2E.md)). Это тест bootstrap-контракта, не «жизни» хоста после деплоя. Unit-тестов на задачи плейбука нет (политика [`ARCHITECTURE.md §1.3`](ARCHITECTURE.md#13-политика-тестирования)).
...
| **`systemd --user` без root-оркестрации**      | Оркестрация FSM — через `systemd --user`. `loginctl enable-linger` — штатная root-задача после bootstrap; опционально Polkit (`threlium_polkit_agent_systemd_enabled`) для privileged `cli_exec` через system `systemd-run`. |
...
| 4.2.13   | `loginctl enable-linger`          | User-manager без логина.                                                                                                                |
| 4.2.13a  | Polkit (opt-in)                   | `threlium_polkit_agent_systemd_enabled`: `policykit-1`, rules в `/etc/polkit-1/rules.d/`, restart `polkit`, acceptance (`systemd-run --wait --pipe`, `daemon-reload` с `--uid=0`). Только `deploy`, не `refresh`. |
| 4.2.13b  | CLI sandbox acceptance            | После linger: `tasks/cli_sandbox_acceptance.yml` — `ProtectSystem=strict` (read `/etc`, write denied). Только `deploy`. |
...
| 4.2.17   | `refresh` (см. §12A)             | ``--tags refresh``: цепочка ``deploy``+``refresh`` в ``site.yml`` (код, env, шаблоны; **без** ``pip``) + ``never``+``refresh`` в ``tasks/refresh.yml`` (сброс Maildir/notmuch/LightRAG, рестарт user-units). HTTP-mock OpenAI тегом **не** поднимается — compose-сервис `wiremock` ([E2E.md §4.4](E2E.md#44-wiremock-openai-http-mock-e2e)). |
...
- **Агент** — через стадию `cli_exec` (`privileged: true` для system scope, [`ARCHITECTURE.md §6`](ARCHITECTURE.md#6-слой-cli-и-безопасность-исполнения)) и transient scope (`systemd-run --scope`, [`ORCHESTRATION.md §5`](ORCHESTRATION.md#5-гонки-восстановление-лимит-параллелизма)). Применение — `daemon-reload`/`restart` из той же цепочки + локальный `git commit`. Профиль может запретить доступ к `threlium_repo_path` / `systemctl --user` — тогда эволюция идёт только через оператора.
...
| [SUBAGENT_TABLE.md](SUBAGENT_TABLE.md) / [MEMORY_TABLE.md](MEMORY_TABLE.md) | Правила роутеров и переходов памяти — плейбук раскладывает реализующие их скрипты. |
| [E2E.md](E2E.md)               | Контракт e2e-тестов плейбука: Testcontainers + Compose + GreenMail + WireMock.                                |
