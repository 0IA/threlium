...
| `threlium-bridge@.service` (единый template) | Мост внешнего канала: `python -m threlium.runners.bridge %i`, `%i` ∈ `email` \| `telegram` \| `matrix`. **`After=`** + **`Wants=`** `threlium-engine.service` (без `Requires`/`BindsTo`). `EnvironmentFile=-…/config/bridge-%i.env`. Раннер импортирует `threlium.bridges.%i`, доставляет каноническое письмо в `fdm`. Email: IMAP IDLE (`imap-tools`), UNSEEN → notmuch-дедуп → канонизация; Telegram/Matrix: long-poll / `matrix-nio` `/sync`. `[Unit]` `StartLimitIntervalSec=0`; `[Service]` `Restart=on-failure`, `RestartSec=1s`. |
