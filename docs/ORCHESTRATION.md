...
| Юнит                                        | Назначение                                                                                                                                                                                            |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `threlium-work@.service` (единый template)  | `Type=exec`: `ExecStart=… python -m threlium.runners.engine_submit %i` (§3), `%i = <stage>:<thread_id>`; `[Unit]` **`BindsTo=`**/**`After=`** `threlium-engine.service`, **`OnSuccess=threlium-sweep@%i.service`**, `StartLimitIntervalSec=0`. FSM в **`threlium-engine.service`**. `[Service]`: `Restart=on-failure`, `RestartSec=1s` — см. [§5](#5-гонки-восстановление-лимит-параллелизма). |
| `threlium-engine.service`                   | Долгоживущий процесс FSM: `python -m threlium.runners.engine`, `[Unit]` `StartLimitIntervalSec=0`; `[Service]` `Restart=always`, `RestartSec=1s`, `Type=notify`, `TimeoutStartSec≥150`, UNIX-сокет `$THRELIUM_HOME/locks/threlium-engine.sock`. После bind — `READY=1`; рантайм — `STATUS=` (движок, LightRAG). Обрабатывает запросы от submit (JSON строка → `process_thread_message`). |
...
| `threlium-bridge@.service` (единый template) | Мост внешнего канала: `python -m threlium.runners.bridge %i`, `%i` ∈ `email` \| `telegram` \| `matrix`. **`After=`** + **`Wants=`** `threlium-engine.service` (без `Requires`/`BindsTo`). `EnvironmentFile=-…/config/bridge-%i.env`. Раннер импортирует `threlium.bridges.%i`, доставляет каноническое письмо в `fdm`. Email: IMAP IDLE (`imap-tools`), UID-watermark (`imap_uid` из notmuch) → `UID SEARCH UID <wm+1>:*` → notmuch-дедуп → канонизация; Telegram/Matrix: long-poll / `matrix-nio` `/sync`. `[Unit]` `StartLimitIntervalSec=0`; `[Service]` `Restart=on-failure`, `RestartSec=1s`. |
