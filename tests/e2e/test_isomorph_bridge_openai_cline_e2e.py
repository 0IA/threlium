...
#: Уникальный per-run токен промпта (uuid-суффикс) → thread-root уникален каждый прогон, треды НЕ
#: пересекаются с прошлыми/соседними прогонами → чистка не нужна (как динамический correlation_key mailflow).
_MARKER = f"isomorph-openai-cline-e2e-{uuid.uuid4().hex[:12]}"
...
def isomorph_cline(e2e_runtime: E2EComposeRuntime) -> Generator[E2EComposeRuntime, None, None]:
    ...
    # Чистки прошлых тредов НЕТ: _MARKER уникален per-run (uuid) → пересечений нет. Прежний
