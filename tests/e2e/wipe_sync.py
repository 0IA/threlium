...
Требует фикстуру ``compose_stack`` (attach-only) — стек должен быть уже поднят
(``wipe_bake.py``); сам ничего не поднимает и не пекёт.
...
и оставляет SUT поднятым. Так последующие прогоны тестов (attach-only + session cold
reset, который ещё раз флашит Maildir/notmuch/LightRAG и рестартит сервисы) стартуют
на чистом работающем стеке без bake/подъёма.
"""
