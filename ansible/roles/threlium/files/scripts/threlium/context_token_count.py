...
def backpack_token_total(msg: EmailMessage, tokenizer: Tokenizer) -> int:
    """Сумма токенов всех непустых leaf-частей enrich-backpack (``iter_backpack_leaf_texts``)."""
    from threlium.mime_reform import iter_backpack_leaf_texts

    return sum(count_tokens(tokenizer, t) for t in iter_backpack_leaf_texts(msg))
