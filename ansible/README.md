...
`cli_exec` тогда при вершине `X-Threlium-Capabilities` из списка вызывает `systemd-run --wait --pipe --uid=0` (system manager). Обычные allowlist-команды по-прежнему идут через `systemd-run --user --scope`. Polkit **не** заменяет `cli_intent` / allowlist.
